...
export default function Charts_handling({selectionUpdated , selectedOption}) {
 ...
 const charts = [`January Load Profile: ${selectedBuilding}` , `Monthly Peak and Average Demand: ${selectedBuilding}` , `Load Profile for ${selectedBuilding}: Monthly Variation` , `Load Profile for ${selectedBuilding}: Daily Variation`];
  ...
  const data = [
    {
      ...
      hovertext:response?.MonthlyPeakDemand.map((demand) => `Monthly Peak Demand: ${demand}`)
    },
    {
      x: response?.Month,
      y: MaxDemandShaving,
...
const darkThemeChartStylings =theme === "dark" ? {paper_bgcolor : "#1F2937" , font : {color : "#94A3B8"} , plot_bgcolor : "#1F2937"} : {};
  return (
    <div className={stylings[theme].chartSelection.parent}>
     <div className={stylings[theme].chartSelection.chartSelectorContainer.parent }>
      <span className=' text-[1.3rem] font-semibold text-slate-400'>Chart Selection</span>
      <div className="w-full flex flex-col items-center">
       {charts.map((val , id)=>
       {
        ...
        {
         setSelected(id);
        }} key={id} className={selected === id ? stylings[theme].chartSelection.chartSelectorContainer.button.selected : stylings[theme].chartSelection.chartSelectorContainer.button.notSelected}>
         <span className={stylings[theme].chartSelection.chartSelectorContainer.span}>{val}</span>
        </button>
       })}
      </div>
     </div>
     ...
     {loaded ?      <div style={{transform : `translateY(${-selected*38}rem)`}} className=" w-full ease-in-out duration-500 h-[400%] shrink-0 flex flex-col items-center">
        ...
        <div className=" MonthlyChart relative w-full h-[25%] shrink-0 flex flex-col items-center justify-center">
          ...
          {plotData && <div className=' w-full translate-y-[-20px] h-full absolute top-0 left-0 flex flex-col items-center justify-center z-[1]'>
            <Plot data={plotData} layout={{...BarGraphlayout , height : 600 , width : 790 , ...darkThemeChartStylings }}/>
            </div>}
        </div>
        <div className=" MonthlyChart relative w-full h-[25%] shrink-0 flex flex-col items-center justify-center">
          <div className="w-full h-full flex flex-col items-center justify-center gap-[1rem]">
          ...
          {daily3Data?.data && <div className=' w-full  translate-y-[-20px] h-full absolute top-0 left-0 flex flex-col items-center justify-center z-[1]'>
            <Plot data={month3Data.data} layout={{...month3Data.layout , height : 600 , width : 790 , template : {...month3Data.layout.template , layout : {...month3Data.layout.template.layout , ...darkThemeChartStylings}}}}/>
            </div>}
        </div>
        <div className=" MonthlyChart relative w-full h-[25%] shrink-0 flex flex-col items-center justify-center">
          <div className="w-full h-full flex flex-col items-center justify-center gap-[1rem]">
          ...
          {daily3Data?.data && <div className=' w-full  translate-y-[-20px] h-full absolute top-0 left-0 flex flex-col items-center justify-center z-[1]'>
            <Plot data={daily3Data.data} layout={{...daily3Data.layout , height : 600 , width : 790 , template : {...daily3Data.layout.template , layout : {...daily3Data.layout.template.layout , ...darkThemeChartStylings}}}}/>
            </div>}
        </div>
